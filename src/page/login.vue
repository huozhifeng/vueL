  ...
  export default {
    ...
    methods: {
      handleSubmit1(ev) {
        var _this = this;
        console.log(this);
        console.log(_this.$refs.ruleForm2);
        ...
        _this.$refs.ruleForm2.validate((valid) => {
          if (valid) {
            ...
            _this.$http.get('/src/data/permissions.json').then(response => {
              // get body data
              var someDatas = JSON.parse(response.bodyText);
              console.log(response)
              var someData;
              for(var i=0;i<someDatas.length;i++){
                if(loginParams.username == someDatas[i].username){
                  someData=someDatas[i]
                }
              }
              if (loginParams.username == someData.username && loginParams.password == someData.password) {
                _this.logining = false;
                sessionStorage.setItem('user', JSON.stringify(loginParams));
